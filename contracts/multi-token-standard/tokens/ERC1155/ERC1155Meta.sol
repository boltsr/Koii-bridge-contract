...
contract ERC1155Meta is ERC1155, SignatureValidator {
    ...
    function metaSafeTransferFrom(
        ...
        if (_isGasFee) {
            (gasReceipt, transferData) = abi.decode(
                signedData,
                (GasReceipt, bytes)
            );
    ...
    function _signatureValidation(
        ...
        bytes32 hash =
            hashEIP712Message(
                keccak256(
                    abi.encodePacked(_encMembers, nonce, keccak256(signedData))
                )
            );

        // Complete data to pass to signer verifier
        bytes memory fullData =
            abi.encodePacked(_encMembers, nonce, signedData);
    ...
    function _transferGasFee(address _from, GasReceipt memory _g) internal {
        ...
        feeRecipient = _g.feeRecipient == address(0)
            ? msg.sender
            : _g.feeRecipient;
